...
1、安装 HomeBrew 并用他安装 App 和 Cli 工具。

- App 可以在 homebrew-cask — Homebrew Formulae 里找有没有
- Cli 工具可以在 homebrew-core — Homebrew Formulae 找有没有。
...
我在 ~/Documents 下建了两个目录（新电脑还没施工完成，这里还会补充更多目录），Code 和 SoftwareConfiguration，长这样。

- Code 用于存代码；
- SoftwareConfiguration 用于保存各种软件配置，我把 Alfred、Eagle 和 Espanso 的配置放在这里。
...
先配置 iTerm2，这是效果图。1）Appearance 里，Pane 里不要「Show per-pane title bar with split panes」，Dimming 里选上第一和第三个，2）Profiles 里，Working Directory 里选「Reuse previous session's directory」。
